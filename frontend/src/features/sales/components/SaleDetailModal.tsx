...
import type { Sale, SaleDetail, SaleStatus } from '../models/types'
...
export default function SaleDetailModal({
  ...
  const details: SaleDetail[] = sale.details ?? []
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-7 scrollbar-thin scrollbar-thumb-emerald-200 scrollbar-track-gray-100">
          ...
          <section>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              <InfoCard label="Cliente"  value={sale.customer_name} />
              <InfoCard label="Ciudad"   value={sale.customer_city} />
              <InfoCard label="Tipo"     value={sale.customer_type} />
              <InfoCard label="Vendedor" value={sale.user_name} />
              <InfoCard label="Fecha"    value={sale.sale_date} />
              <div className="flex flex-col gap-1 bg-gray-50 border border-gray-100 rounded-xl px-4 py-3">
                ...
                <span
                  className={`inline-flex items-center self-start mt-0.5 px-2.5 py-0.5 rounded-full text-xs font-medium border ${
                    statusColors[sale.status] ?? 'bg-gray-100 text-gray-700'
                  }`}
                >
          ...
          <section>
            ...
            <div className="grid grid-cols-3 gap-3">
              <FinanceCard label="Subtotal" value={fmtCOP(sale.subtotal)} />
              <FinanceCard label="IVA"      value={fmtCOP(sale.iva)} />
              <FinanceCard label="Total"    value={fmtCOP(sale.total)} highlight />
            </div>
          </section>
                    ...
                    {details.map((det, i) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 font-medium text-gray-800">
                          {det.detail_roasted_coffee?.product?.name
                            ?? `Producto #${det.detail_roasted_coffee_id}`}
                        </td>
