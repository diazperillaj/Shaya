...
import { SaleFields } from "./models/fields";
import type { Sale } from "./models/types";
import { useSalesService, getProducts } from "./services/sale.api";
...
export default function SalesPage() {
  ...
  const { fetchSales } = useSalesService();
  const { updateSale } = useSalesService();
  const { deleteSale } = useSalesService();
  ...
  const loadSales = async (): Promise<void> => {
    const Sales = await fetchSales({ search });
    setData(Sales);
  };
  ...
  const [fields, setFields] = useState(SaleFields);

  useEffect(() => {
      const loadProducts = async () => {
        const products = await getProducts();
  
        const productOptions = [{ label: "Seleccionar", value: "" }, ...products];
  
        setFields((prev) =>
          prev.map((f) =>
            f.accessor === "product" ? { ...f, options: productOptions } : f,
          ),
        );
      };
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <div className="flex gap-2 flex-col md:flex-row w-max">
            {/* <button
                            onClick={loadSales} // 🔹 botón Filtrar recarga los datos
