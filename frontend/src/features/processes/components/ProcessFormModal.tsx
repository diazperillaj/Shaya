...
export default function ProcessFormModal({
  ...
  useEffect(() => {
    ...
    setInvoiceNumber(initialProcess.invoice_number ?? "");
    setProcessDate(initialProcess.process_date ?? "");
    setParchmentId(String(initialProcess.parchment_id ?? ""));
    setParchmentKg(initialProcess.parchment_kg ?? 0);
    setObservations(initialProcess.observations ?? "");
  ...
  const handleSave = async () => {
    ...
    if (details.length === 0 || details.some((d) => !d.product_id || d.product_id <= 0))
      return setError("Todos los detalles deben tener un producto seleccionado.");
    if (details.some((d) => d.bag_quantity <= 0 ))
  ...
  const handleDelete = async () => {
    ...
    const confirmed = window.confirm(
      "Esta accion borrara el proceso y todos sus detalles. Deseas continuar?",
    );
    if (!confirmed) return;
    ...
    try {
      ...
      setError(err.message || "Error al borrar el proceso.");
    } finally {
      setDeleting(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[95vh] flex flex-col">
        ...
        <div className="bg-gradient-to-r from-emerald-900 via-emerald-800 to-emerald-900 rounded-t-2xl px-6 py-5 flex items-center justify-between flex-shrink-0">
          <h2 className="text-lg font-semibold text-white flex items-center gap-3">
            ...
            {isEdit ? "Editar proceso de café tostado" : "Nuevo proceso de café tostado"}
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-4 bg-gray-50 rounded-b-2xl border-t border-gray-100 flex-shrink-0 flex justify-between items-center gap-3">
          ...
          >
            ...
            {isEdit ? "Actualizar proceso" : "Guardar proceso"}
          </button>
        </div>
      </div>
    </div>
...
function DetailLine({
  ...
  const gramsPorBolsa = selectedProduct?.quantity ?? detail.grams_per_bag ?? 0;
    ...
    <div className="bg-gray-50 border border-gray-100 rounded-2xl p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <FormField label="Producto" required>
            <select
              ...
              onChange={(e) => {
                ...
                onChange("product_id", selectedId);
                if (product) onChange("grams_per_bag", product.quantity);
              }}
              className={inputCls}
            >
              <option value="" disabled>
        ...
        <FormField label="Bolsas">
          <input
            ...
            onChange={(e) => {
              ...
              if (selectedProduct) onChange("grams_per_bag", selectedProduct.quantity);
            }}
            placeholder="0"
            className={inputCls}
          />
        ...
        <FormField label="Gramos / bolsa">
          <div className={readonlyCls}>{gramsPorBolsa}</div>
        </FormField>
