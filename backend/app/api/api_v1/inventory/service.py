...
class ParchmentService:
    ...
    def create_parchment(self, parchment_data: ParchmentCreate) -> Parchment:
        ...
        inventory = Inventory(
            product_id=parchment_data.product_id,
            date=datetime.now(),
        ...
        full_price = parchment_data.purchase_price * parchment_data.initial_quantity
        ...
        parchment = Parchment(
            ...
            purchase_price=parchment_data.purchase_price,
    ...
    def update_parchment(self, parchment_id: int, parchment_data: ParchmentUpdate) -> Parchment:
        ...
        Las cantidades NO se actualizan aquí, solo mediante movimientos.
        ...
        if parchment_data.variety is not None:
            parchment.variety = parchment_data.variety

        if parchment_data.altitude is not None:
            parchment.altitude = parchment_data.altitude

        if parchment_data.humidity is not None:
            parchment.humidity = parchment_data.humidity

        if parchment_data.origin_batch is not None:
            parchment.origin_batch = parchment_data.origin_batch
